...
document.getElementById('signup2').addEventListener('click',  async function (nomeinput, emailinput, pswinput) {
    ...
    if (verificaEmail(emailinput) && verificaSenha(pswinput)) {
        const json = await fetch('https://web-api-dicionario-simples.herokuapp.com/Users/signup', {
            method: 'post',
...
document.getElementById('signin2').addEventListener('click', async function (emailinput, pswinput) {
    ...
    pswinput = document.getElementById('user_password');
    if (verificaEmail(emailinput) && verificaSenha(pswinput)) {
        const json = await fetch('https://web-api-dicionario-simples.herokuapp.com/Users/signin', {
            method: 'post',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
                email: emailinput.value,
